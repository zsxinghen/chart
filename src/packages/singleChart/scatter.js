...
export default {
  ...
  setSymbleSize(config){
    if(config.series.sizeType=='set'){
         return{
          symbolSize:config.series.size
         }
    }
    else{
      return {
        symbolSize: (val)=> {
          return val / config.series.scale;
      },
      }
    }
  ...
  setSeries(config, data) {
    ...
    for (let i in data.valArr) {
      let obj = {
        name: data.nameArr[i],
        type: config.series.type?'effectScatter':'scatter',
